...
class Player:
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        if len(box_lists) == 0:
            distance1 = 1.0
            distance2 = 1.0
            hole1 = np.random.uniform(low=0, high=1, size=1)
            hole2 = np.random.uniform(low=0, high=1, size=1)

        elif len(box_lists) == 1:
            distance1 = (box_lists[0].x - agent_position[0]) / CONFIG['WIDTH']
            distance2 = 1.0
            hole1 = (box_lists[0].gap_mid - agent_position[1]) / CONFIG['HEIGHT']
            hole2 = np.random.uniform(low=0, high=1, size=1)

        else:
            distance1 = (box_lists[0].x - agent_position[0]) / CONFIG['WIDTH']
            distance2 = (box_lists[1].x - agent_position[0]) / CONFIG['WIDTH']
            hole1 = (box_lists[0].gap_mid - agent_position[1]) / CONFIG['HEIGHT']
            hole2 = (box_lists[1].gap_mid - agent_position[1]) / CONFIG['HEIGHT']

        velocity_normal = velocity / 10

        input_vector = np.array(([distance1], [distance2], [hole1], [hole2], [velocity_normal]))

        nn_result = self.nn.forward(input_vector)
